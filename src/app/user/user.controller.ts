...
export class UserController {
	...
	public async subscription(
		...
		try {
			...
			await this.userService.subscription.patch(
	...
	public async onUserRegisterVerify(user: UserEntity): Promise<void> {
		try {
			...
			await mail({
				...
				to: user.email,
				html: await renderTemplate(`/mail/welcome.twig`, { user, config }),
			});
	...
	public async onUserPasswordNew(user: UserEntity): Promise<void> {
		try {
			...
			await mail({
				...
				to: user.email,
				html: await renderTemplate(`/mail/password_new.twig`, { user, config }),
			});
